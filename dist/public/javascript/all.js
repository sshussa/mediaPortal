...
$(window).load(function () {
    ...
    $('article.revealtray').click(function (event) {
        pde(event);
        var article = $(this);
        ...
        if (trayOpen == false) {
            ...
            if (article.parent().hasClass('active')) {
                closeTray(blockHeight);
                return;
            }
            closeTray(openTray, article, blockHeight);
        }
    });

    function openTray(article) {
        trayOpen = true;
        var extend = $(article).parent().find('.extend');
        var total_height = article.outerHeight(true) + $(extend).height();
        var headerHeight = $("nav").outerHeight();
        $(article).parent().height(total_height).addClass('active');
        $(extend).slideDown(400, function () {
            $.scrollTo($(article).offset().top - headerHeight, 800, { axis: 'y' });
        });
    }

    function closeTray(callback, article, blockHeight) {

        var active = $('.revealtray li.active');

        $(active).animate({ 'height': '150' }, 400, 'swing').removeClass('active');
        $(active).find('.extend').slideUp(400, function () {
    ...
    $('.feature-item').hover(function () {
        $(this).find('.scale-effect').stop().animate({ "background-size": "110%" }, 300);
    }, function () {
        $(this).find('.scale-effect').stop().animate({ "background-size": "100%" }, 300);
    });

    $('.touch .feature-item').click(function () {
        $(this).toggleClass('feature-item-active');
    });

    function nextperson() {
        $('.data-container').empty();
        var t = $(".current-section").find(".person.-active"),
            r = t.next(".person").length > 0 ? t.next(".person") : $(".current-section .person:first");
        gridcarousel(r), $(".active-container .timeline-item").addClass("enterRight");
    };

    function previousperson() {
        $('.data-container').empty();
        var t = $(".current-section").find(".person.-active"),
            r = t.prev(".person").length > 0 ? t.prev(".person") : $(".current-section .person:last");
        gridcarousel(r), $(".active-container .timeline-item").addClass("enterLeft");
    };

    function gridcarousel(r) {
        $('.person').removeClass('-active');
        r.find('.timeline-info').clone().prependTo('.data-container');
        r.closest('.person').addClass('-active');
    }

    $('.person-activate').click(function () {
        timelinesection = $(this).closest('.timeline').addClass('current-section');
        var timelineHeading = $(this).parents().prev('h3').html();
        var w = $(window).scrollTop();
        var a = $(this).closest(".person");
        var c = $(this).closest(".person").offset().top;
        var d = $(".leadership-grid").offset().top;
        var b = c - w;

        if (b == 0) {
            b = -228;
        }
        var s = -($(".leadership-grid").offset().top - a.offset().top);
        ...
        $(this).find('.timeline-info').clone().prependTo('.data-container');
        //$(this).parents().prev('h3').html().prependTo('.active-person-header');
        $(".active-timeline-header").empty();
        $(".active-timeline-header").append(timelineHeading);
        $('.data-container').show();
        $('.active-container').addClass('active');
        $('.global-overlay').show();
        $('.person').removeClass('-active');
    ...
    $('.next-person').click(function () {
        ...
        $('.active-container .timeline-item').addClass("exitLeft)"), setTimeout(function () {
            nextperson(timelinesection);
        }, 200);
    });

    $('.previous-person').click(function () {
        $('.active-container .timeline-item').addClass("exitRight)"), setTimeout(function () {
            previousperson(timelinesection);
        }, 200);
    });

    $('.active-container .plus-icon').click(function () {
        $('.timeline').removeClass('current-section');
        $('.data-container').empty();
        $('.data-container').hide();
        $('.active-container').removeClass('active');
        $('.person').removeClass('-active');
        $('.leadership-grid').removeClass('-active');
        $('.global-overlay').hide();
    ...
    var images = ['fourcheeseravioli.jpg', 'maggianos_food_hero.jpg', 'burgertron.jpg', 'primavera.jpg', 'appetizer_tray.jpg', 'freshberrytiramasu.jpg', 'shortribfilet.jpg'];
...
/*    $(".brinkertron").backstretch('/assets/img/' + image, {
        centeredY: true
    });*/
});
"use strict";
    ...
    "object" === typeof d && (b = d, d = 0);"function" === typeof b && (b = { onAfter: b });"max" === a && (a = 9E9);b = $.extend({}, p.defaults, b);d = d || b.duration;var u = b.queue && 1 < b.axis.length;u && (d /= 2);b.offset = h(b.offset);b.over = h(b.over);return this.each(function () {
      function k(a) {
        ...
        var l = n(this),
            ...
            e = a,
            f = {},
            t;switch (typeof e) {case "number":case "string":
            if (/^([+-]=?)?\d+(\.\d+)?(px|%)?$/.test(e)) {
              e = h(e);break;
